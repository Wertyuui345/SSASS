...
class BeamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = np.asarray(np.squeeze(data), dtype=complex)
        mag, _ =librosa.magphase(data)

        mel_transform = T.MelScale(sample_rate=44100, n_stft = 512 // 2 + 1)
...
class BeamSep(nn.Module):
    def __init__(self):
        ...
        self.conv1=ConvLayer(24, 32) #8 input channels(change to # of beamspace input channels) Should be 15? I believe
...
for epoch in range(200):
    ...
    checkpoint(model, optimizer, f"checkpointmel.pth", epoch, running_loss / len(train_loader))
...
model_path = 'AudioMaskerCircularMEL-Small.pth'
