...
class BeamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = torch.from_numpy(np.concatenate((data.real, data.imag), axis = 0)).float()
...
class BeamSep(nn.Module):
    def __init__(self):
        ...
        self.pool=nn.AvgPool2d((1,30)) #average along time axis (38)
        #Looks like the paper averages everything once and keeps freq the same
        self.flatten = nn.Flatten(start_dim=1)
        self.final1=FullCon(4608, 257) #Change 2048, to appropriate out size
...
#   checkpoint = torch.load(wandb.restore('checkpoint.pth'))
...
for epoch in range(200):
    ...
    checkpoint(model, optimizer, f"checkpointfull257.pth", epoch, running_loss / len(train_loader))
...
model_path = 'AudioMaskerCircular-Full.pth'
