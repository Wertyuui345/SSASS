...
class BeamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = np.moveaxis(beamfile['training_data'][2]
            .reshape(50, 257, 15), (0, 1, 2), (2, 1, 0))
            #beamspace transform, removing 0 HZ
        x = torch.from_numpy(np.concatenate((x.real, x.imag), axis = 0)[:, 1:257, :]).float()
        y = torch.from_numpy(beamfile['target'][25, 1:257]).float()
...
train_dataset = BeamDataset(files)
...
Time Length = 50
Spectrogram Filter Banks = 256(7)(?)
...
class BeamSep(nn.Module):
    def __init__(self):
        ...
        self.conv1=ConvLayer(30, 16) #8 input channels(change to # of beamspace input channels) Should be 15? I believe
        ...
        self.final1=FullCon(2048, 256) #Change 2048, to appropriate out size
        self.final2=Head(256, 256, nn.Sigmoid)
...
# lr_scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.01)
...
input = torch.randn((64, 30, 256, 50), dtype=torch.float).to(device)
out = model(input)
...
for epoch in range(50):
    ...
    if (epoch % 20 == 0 and epoch != 0):
        checkpoint(model, optimizer, f"epochtest-{epoch}.pth", epoch, running_loss / len(train_loader))
...
model_path = 'AudioMasker-Small.pth'
